...
class SubSectionOwnerController extends Controller
{
    use CompanyCommons;
    ...
    public function sectionOwnerInfo(Request $request)
    {
        ...
        $owners = UserSection::select('user_id')->where(['comp_id' => $comp_id, 'section_id' => $section_id, 'standard_id' => $standard_id, 'enable' => 1])->distinct()->get();

        $owners = $owners->map(function ($user) {
            return $user->user_id;
        });

        $owner_teams = OwnerTeamSection::select('team_id')->where(['comp_id' => $comp_id, 'section_id' => $section_id, 'standard_id' => $standard_id, 'enable' => 1])->distinct()->get();

        $owner_teams = $owner_teams->map(function ($team) {
            return $team->team_id;
        });

        $owner_thirdparty = ThirdPartyOwnerSections::select('tp_id')->where(['comp_id' => $comp_id, 'section_id' => $section_id, 'standard_id' => $standard_id, 'enable' => 1])->distinct()->get();

        $owner_thirdparty = $owner_thirdparty->map(function ($team) {
            return $team->tp_id;
        });

        $custodians = CustodianSection::select('user_id')->where(['comp_id' => $comp_id, 'section_id' => $section_id, 'standard_id' => $standard_id, 'enable' => 1])->distinct()->get();

        $custodians = $custodians->map(function ($user) {
            return $user->user_id;
        });

        $custodian_teams = CustodianTeamSection::select('team_id')->where(['comp_id' => $comp_id, 'section_id' => $section_id, 'standard_id' => $standard_id, 'enable' => 1])->distinct()->get();

        $custodian_teams = $custodian_teams->map(function ($team) {
            return $team->team_id;
        });

        $custodian_thirdparty = ThirdPartyCustodianSections::select('tp_id')->where(['comp_id' => $comp_id, 'section_id' => $section_id, 'standard_id' => $standard_id, 'enable' => 1])->distinct()->get();

        $custodian_thirdparty = $custodian_thirdparty->map(function ($team) {
            return $team->tp_id;
        });

        $authority = CompanySection::where(['comp_id' => $comp_id, 'section_id' => $section_id, 'standard_id' => $standard_id])->first();
    ...
    public function saveSectionOwnerInfo(Request $request, SubSectionCustodianController $subSectionCustodianController)
    {
        $this->validate($request, [
            'comp_id' => 'required',
            'standard_id' => 'required',
        ...
        if ($owner_type === 'users') {
            ...
            $this->saveOwners($all_users, $user, $comp_id, $standard_id, $csection, $owners);
            $this->removeTeamOwners($all_teams, $comp_id, $standard_id, $csection, $owner_teams);
            $this->removeTPOwners($all_thirdparties, $comp_id, $standard_id, $csection, $owner_thirdparty);
        } elseif ($owner_type === 'teams') {
            // remove users and assign teams
            $this->saveTeamOwners($all_teams, $user, $comp_id, $standard_id, $csection, $owner_teams);
            $this->removeOwners($all_users, $comp_id, $standard_id, $csection, $owners);
            $this->removeTPOwners($all_thirdparties, $comp_id, $standard_id, $csection, $owner_thirdparty);
        } else {
            // renove users, teams & assign third party
            $this->saveTPOwners($all_thirdparties, $user, $comp_id, $standard_id, $csection, $owner_thirdparty);
            $this->removeOwners($all_users, $comp_id, $standard_id, $csection, $owners);
            $this->removeTeamOwners($all_teams, $comp_id, $standard_id, $csection, $owner_teams);
        }
        ...
        if ($custodian_type === 'users') {
            // remove team and assign users
            $subSectionCustodianController->saveCustodian($all_users, $user, $comp_id, $standard_id, $custodians, $csection);
            $subSectionCustodianController->removeTeamCustodian($all_teams, $comp_id, $standard_id, $csection, $custodian_teams);
            $subSectionCustodianController->removeTPCustodians($all_thirdparties, $comp_id, $standard_id, $csection, $custodian_thirdparty);
        } elseif ($custodian_type === 'teams') {
            // remove users and assign teams
            $subSectionCustodianController->saveTeamCustodian($all_teams, $user, $comp_id, $standard_id, $csection, $custodian_teams);
            $subSectionCustodianController->removeCustodian($all_users, $comp_id, $standard_id, $custodians, $csection);
            $subSectionCustodianController->removeTPCustodians($all_thirdparties, $comp_id, $standard_id, $csection, $custodian_thirdparty);
        } else {
            $subSectionCustodianController->saveTPCustodians($all_thirdparties, $user, $comp_id, $standard_id, $csection, $custodian_thirdparty);
            $subSectionCustodianController->removeCustodian($all_users, $comp_id, $standard_id, $custodians, $csection);
            $subSectionCustodianController->removeTeamCustodian($all_teams, $comp_id, $standard_id, $csection, $custodian_teams);
        }

        CompanySection::where(['comp_id' => $comp_id, 'section_id' => $csection['id'], 'standard_id' => $standard_id])
    ...
    public function saveOwners($all_users, $user, $comp_id, $standard_id, $csection, $owners)
    {
    ...
    public function removeOwners($all_users, $comp_id, $standard_id, $csection, $owners)
    {
        if (count($all_users) > 0) {
            foreach ($all_users as $user_id) {
    ...
    public function saveTeamOwners($all_teams, $user, $comp_id, $standard_id, $csection, $teams)
    {
    ...
    public function removeTeamOwners($all_teams, $comp_id, $standard_id, $csection, $teams)
    {
        if (count($all_teams) > 0) {
            foreach ($all_teams as $team_id) {
    ...
    public function saveTPOwners($all_thirdparties, $user, $comp_id, $standard_id, $csection, $owner_thirdparty)
    {
        if (count($all_thirdparties) > 0) {
            foreach ($all_thirdparties as $tp_id) {

                $aSection = $this->tpOwnerSectionAssigned($csection['id'], $tp_id, $comp_id, $standard_id);
    ...
    public function removeTPOwners($all_thirdparties, $comp_id, $standard_id, $csection, $owner_thirdparty)
    {
        if (count($all_thirdparties) > 0) {
            foreach ($all_thirdparties as $tp_id) {
    ...
    public function assignNewTPOwnerSection($section_id, $tp_id, $comp_id, $standard_id, $assigned_by, $parent)
    {
