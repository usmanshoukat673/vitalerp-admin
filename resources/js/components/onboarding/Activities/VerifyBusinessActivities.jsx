...
import CustomBusinessActivity from './CustomBusinessActivity';
import AddBusinessActivity from './AddBusinessActivity';

const VerifyBusinessActivities = ({ match, history }) => {
    ...
    useEffect(() => {
        if (!_.isEmpty(build_id)) {
            setLoading(true);
            axios.get(`/api/auth/onboarding/business-activities/${build_id}`)
                .then(e => {
                    setLoading(false);
                    setErrors([]);
    ...
    const handleBack = () => {
        history.push(`/verify-bussiness-departments/${build_id}`);
    }
        ...
        <div className="auth_page_content">
            <Box sx={{ display: 'flex', marginTop: '30px' }}>
                ...
                <Box sx={{ flex: '0.6', padding: '20px', borderRadius: '6px' }}>
                    <Typography variant="h4">Verify Business Activities</Typography>
                    <div className="hr_line"></div>
                    <form>
                        {
                            _.size(activities) > 0 && <Box sx={{ padding: '0px 10px 15px 10px', borderRadius: '6px' }}>
                                {
                                    _.map(activities, activity => (
                                        <BusinessActivity key={activity.id} activity={activity} />

                                    ))
                                }
                            </Box>
                        }
                        <p style={{ paddingTop: '10px' }}>Please verify key business activities before continuing </p>
                        <Box sx={{ padding: '15px 10px', borderRadius: '6px' }}>
                            <Typography variant="h5" sx={{ color: '#000', paddingBottom: '10px' }}>Add Missing</Typography>
                            {
                                _.map(custom_activities, activity => (

                                    <CustomBusinessActivity delete_activity={handleDeleteActivity} key={activity.id} activity={activity} />

                                ))
                            }
                            <AddBusinessActivity added={handleAddCustomActivity} build_id={build_id} />
                        </Box>
                        <div className="mt-3">
...
export default VerifyBusinessActivities;