...
const AddDeparmentAsset = ({ build_id, deparment_id, added, asset_type, standard_id }) => {
    ...
    const handleSubmit = () => {
        setLoading(true);
        axios.post(`/api/auth/onboarding/add-business-asset`, {
            build_id: build_id,
            name: name,
        ...
        <>
            <Box sx={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                <Box sx={{ flex: '0.8', marginRight: '15px' }}>
                    <TextField label="Asset Type" fullWidth variant="outlined" size='small' value={name} className={classNames(handlerInputError('name'), 'build__input')} onChange={handleNameChange} />
                </Box>
...
export default AddDeparmentAsset;