...
const AddBuildSupplier = ({ build_id, added, standard_id }) => {
    ...
    const handleSubmit = () => {
        setLoading(true);
        axios.post(`/api/auth/onboarding/add-build-supplier`, {
            build_id: build_id,
            name: name,
        ...
        <>
            <Box sx={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                <Box sx={{ flex: '0.8', marginRight: '15px' }}>
                    <TextField label="Suppliers Type" fullWidth variant="outlined" size='small' value={name} className={classNames(handlerInputError('name'), 'build__input')} onChange={handleNameChange} />
                </Box>
...
export default AddBuildSupplier;