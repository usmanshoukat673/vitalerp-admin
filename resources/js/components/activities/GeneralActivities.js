...
import './GeneralActivities.scss';
import _ from 'lodash';
import showTZDate from '../../utils/showTZDate';
import { FiActivity } from "react-icons/fi";
import axiosInstance from '../../api/api';

class GeneralActivities extends Component {
    ...
    loadActivities = activePage => {
        ...
        axiosInstance.get(`/api/user/org/general-activities/${company.id}?page=${activePage}`).then(e => {
    ...
    detectActivity = (activity, company) => {
        if (activity.event_type === 'general_settings') {
            return `${activity.user.first_name} ${activity.user.last_name} changed General Settings at ${showTZDate(activity.created_at, company.timezone)}`;
        }
        else {
            return `${activity.user.first_name} ${activity.user.last_name} uploaded new company logo at ${showTZDate(activity.created_at, company.timezone)}`;
        }
    }
    ...
    render() {
        ...
        return (<div className="activity__bucket">
            <div className="at__bucket__header">General Activities</div>
