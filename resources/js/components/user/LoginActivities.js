...
class LoginActivities extends Component {
    ...
    loadActivities = page => {
        ...
        axiosInstance.get('/api/user/activities/login?page=' + page).then(e => {
        ...
        return (
            _.map(activites, activity => (
                <Message key={activity.created_at}>
                    <Message.Header>{activity.auth}</Message.Header>
    ...
    render() {
        ...
        return (
            <React.Fragment>
                <Segment>
                    <Header size='medium'><FaFingerprint className="change_pwd_key_icon" /> &nbsp; Login History</Header>
...
export default withRouter(connect(null, { clearUser, clearToken })(LoginActivities));
