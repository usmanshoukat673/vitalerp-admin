...
/**
 * Renders a component for delegating control to multiple users.
 *
 * @return {JSX.Element} The rendered component.
 */
const DeligateControl = () => {
    ...
    useEffect(() => {
        setUserIds(control_info.deligated_users ? control_info.deligated_users : []);
    }, [control_info]);
    ...
    const handleSubmit = () => {
        setLoading(true);
        axiosInstance.post(`/api/user/compliance/delegate-control`, {
            ...
            copy_control_info.deligated_users = e.data.deligated_users;
    ...
    const determineCanDeligate = () => {
        return _.size(control_info.assigned_users) > 0;
    }

    return (
        <>
             <div style={{ padding: '0px 0px' }}>
                        <FormControl sx={{ m: 1, minWidth: 350 }} size="small" disabled={!determineCanDeligate()}>
                            <FormLabel>Deligate User(s)</FormLabel>
                            <Select
                                multiple
                        ...
                        <Stack spacing={2} direction="row" sx={{ ml: '9px' }}>
                            <Button disabled={!determineCanDeligate()} onClick={handleSubmit} variant="outlined">Submit</Button>
                        </Stack>
                    </div>
        </>
    )
...
export default DeligateControl;