...
const SupplierDomainsList = () => {
    ...
    useEffect(() => {
        if (assignDomainDialog?.assigned) {
            setPage(1);
            getSuppliers();
            dispatch(setAssignDomainDialog({ assigned: false }));
        }
    }, [assignDomainDialog]);
    ...
    const handleAssignDomainClick = (supplier) => {
        dispatch(setAssignDomainDialog({
            open: true,
            supplier
        }));
    }
                            ...
                            list.map((supplier) => (
                                <React.Fragment key={supplier.id}>
                                    ...
                                    <TableRow sx={{ borderBottom: 'none' }}>
                                        <TableCell align="left" sx={{ borderBottom: 'none', fontWeight: 'bold', backgroundColor: '#f5f5f5' }}>
                                            <Link underline="hover" onClick={() => handleAssignDomainClick(supplier)} style={{ cursor: 'pointer' }}>
                                                {supplier.name}
                                            </Link>
                                        </TableCell>
                                        <TableCell align="center" sx={{ borderBottom: 'none', backgroundColor: '#f5f5f5' }}>{supplier.email}</TableCell>
                                        <TableCell align="center" sx={{ borderBottom: 'none', backgroundColor: '#f5f5f5' }}>{supplier.phone}</TableCell>
                                        <TableCell align="right" sx={{ borderBottom: 'none', backgroundColor: '#f5f5f5' }}></TableCell>
                                        <TableCell align="right" sx={{ borderBottom: 'none', backgroundColor: '#f5f5f5' }}></TableCell>
                                    </TableRow>

                                    {/* Labor Categories Row */}
                                    <TableRow>
                                        <TableCell colSpan={5} sx={{ pl: 4 }}>
                                            {
                                                _.size(supplier.domains) > 0 ? (
                                                    <ul>
...
export default SupplierDomainsList;