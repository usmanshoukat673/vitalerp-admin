...
import { setSelectedTask, setAllProjects, setCompanyUsers, setBackPageURL, setProjectProfile, setAllProjectSubjects } from '../../actions';
...
import projectImg1 from '../../../images/projects/project-1.jpg';
import projectImg2 from '../../../images/projects/project-2.jpg';
import projectImg3 from '../../../images/projects/project-3.jpg';
import projectImg4 from '../../../images/projects/project-4.jpg';
...
const ProjectCard = ({ project, company, projectDetail }) => {
    ...
    return (
        <Card className="d-block">
            ...
            <Card.Body className={project.image ? 'position-relative' : ''}>
                ...
                <h4 className="mt-0">
                    <a onClick={() => projectDetail(project)} className="text-title chand">
                        {project.title}
                    </a>
                </h4>
                ...
                {project.description && (
                    <div className="text-muted font-13 my-3">
                ...
                <p className="mb-1">
                    <span className="pe-2 text-nowrap mb-2 d-inline-block">
                        <i className="mdi mdi-format-list-bulleted-type text-muted me-1"></i>
                        <b>{_.size(project.tasks)}</b> Tasks
                    </span>
                    <span className="text-nowrap mb-2 d-inline-block">
                        <i className="mdi mdi-comment-multiple-outline text-muted me-1"></i>
                        <b>{project.total_comments}</b> Comments
                    </span>
                </p>
                <div>
                    <Link
...
const Projects = ({ company, history, projects, setAllProjects, setBackPageURL }) => {
    ...
    const handleProjectDetailClick = (project) => {
        dispatch(setProjectProfile(project));
        dispatch(setAllProjectSubjects(project.subjects));
        history.push(`/${company.slug}/projects/details/${project.id}`);
    }
    ...
    const newProject = () => {
        let url = `/${company.slug}/projects/list`;
        setBackPageURL(url);
    ...
    return (
        <>
            ...
            <Row className="mb-2">
                <Col sm={4}>
                    <MUIButton onClick={newProject} component="label" variant="outlined" startIcon={<AddIcon />}>
                        Create Project
                    </MUIButton>
                </Col>
                <Col sm={8}>
                ...
                {projects.map((project, i) => {
                    return (
                        <Col md={6} xxl={3} key={'proj-' + project.id}>
                            <ProjectCard project={project} company={company} projectDetail={handleProjectDetailClick} />
                        </Col>
                    );
                })}
                {
            ...
            <RightDrawer title="Projects" component={<RightSidebar />} />
        </>
    );
};
...
export default withRouter(connect(mapStateToProps, { setSelectedTask, setAllProjects, setCompanyUsers, setBackPageURL })(Projects));