...
const TaskComment = ({comment, task, newCommentAdded}) => {
        ...
        <div className="w-100">
...

            {
                _.size(the_comment.replies) > 0 && _.map(the_comment.replies, reply => {
                    return (<TaskComment comment={reply} task={task} key={reply.id} newCommentAdded={newCommentAdded} />)
                })
            }

            {
                replay && <div className="border rounded mt-4">
                    <CommentForm task={task} created={handleCreated} parent={the_comment.id} showCancel={true} cancel={handleOnCancel} />
                </div>
            }
        </div>
    </div>
