...
const QualityCertsProvider = ({ selectedItems, setSelectedItems, entityName }) => {
    ...
    const fetchItems = async (query = '') => {
        setIsLoading(true);
        try {
            const response = await axiosInstance.get(`/api/user/valid-values/quality-certs`, {
                params: { search: query }
            });
    ...
    return (
        <Grid container spacing={2}>
            ...
            <Grid item xs={6} sm={6}>
                <Box sx={{ flex: 1, border: '1px solid #ccc', borderRadius: '5px', padding: '10px' }}>
                    <Typography variant="h6" gutterBottom>Selected {`${entityName}`}:</Typography>
                    <Box sx={{ height: '315px', overflowY: 'auto' }}>
                        {
                            _.size(selectedItems) > 0 ? selectedItems.map(id => {
                                ...
                                <p style={{ color: 'grey', paddingTop: '6px', paddingLeft: '16px' }}><i>No {`${entityName}`} selected</i></p>
                        }
                    </Box>
                </Box>
            </Grid>
...
export default QualityCertsProvider;